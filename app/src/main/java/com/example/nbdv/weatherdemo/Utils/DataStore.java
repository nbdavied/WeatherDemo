...
public class DataStore {
    ...
    private final static String WIDGET_ID_PREFIX = "widget_id_";

    //存储widget configuration
    public static void saveWidgetConf(Context context, int appWidgetId, String data) {
        SharedPreferences.Editor editor = context.getSharedPreferences(DEFAULT_PREFERENCE, Context.MODE_PRIVATE).edit();
        editor.putString(WIDGET_ID_PREFIX + appWidgetId, data);
        editor.commit();
    }

    //获得widget configuration存储的值
    public static String getWidgetConf(Context context, int appWidgetId) {
        SharedPreferences sp=context.getSharedPreferences(DEFAULT_PREFERENCE, Context.MODE_PRIVATE);
        return sp.getString(WIDGET_ID_PREFIX+appWidgetId,null);
    }
    //删除widget conficuration数据
    public static void deleteWidgetConf(Context context,int appWidgetId){
        SharedPreferences.Editor editor = context.getSharedPreferences(DEFAULT_PREFERENCE, Context.MODE_PRIVATE).edit();
        editor.remove(WIDGET_ID_PREFIX+appWidgetId);
        editor.commit();
    }
}
